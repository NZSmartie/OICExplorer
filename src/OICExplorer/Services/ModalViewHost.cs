...
namespace OICExplorer.Services
{
    ...
    {
        public static ReactiveCommand<IRoutableViewModel, IRoutableViewModel> NavigateToModal(this RoutingState routingState)
        {
            return ReactiveCommand.CreateFromTask<IRoutableViewModel, IRoutableViewModel>(async vm => 
            {
                var viewhost = new ModalViewHost();
                await Application.Current.MainPage.Navigation.PushModalAsync(viewhost);
                return await viewhost.Router.Navigate.Execute(vm);
            });
        }
    }

    public class ModalViewHost : NavigationPage, IActivatable, IScreen
    {
        ...
        {
            Locator.CurrentMutable.RegisterConstant<IScreen>(_prevScreen);
        ...
        public ModalViewHost()
        {
            ...
            _prevScreen = Locator.Current.GetService<IScreen>();
            if (_prevScreen == null) throw new Exception("You *must* register an IScreen class representing your App's main Screen");
